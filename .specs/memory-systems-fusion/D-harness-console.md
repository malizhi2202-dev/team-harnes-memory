...
审核: 待审（第1轮 · GD 通过 3/3 ✅（GD-3 产品复审改判），待 ⑤ 人审）

- 事实依据：`GAPS.md` §2 行 8–10；`.specs/CONTEXT.md`（四轴含 harness）与 `CONTEXT.md:323`（**M5 契约真源**：「自研 harness 上报 3 个 header → proxy 消费锚定信号；proxy 侧已消费，harness 侧由用户自研」，标 📄 未落地）；platform 议题 01（Connector Contract V1 八条能力）。GD-2/3 复审勘误：初稿此行的「backlog M5」系指源错误——`memory-platform-backlog.md` 只有 A–E 五组 47 条（`^| M[0-9]` 零命中），已改指 `CONTEXT.md:323`。
...
1. 立项方向：「**Harness 接入与运行观测**」屏（建议路由 `#/harness`，与 D-2 血缘屏分职：那张看一条记忆的来路，这张看一个 harness 的接入态）。**v1 只留两段（GD-1 高级产品经理切段条件②）**：①**实例清单**（注册/能力声明/租约/降级态——`harness_id/instance_id/capability_profile/lease_id/revoked_at` 已在 `_build/sec3.html:249` 字段行，含租约过期与撤销呈现）；②**会话↔空间绑定与就地改挂**（今天确实无处回答；参照 mem:session-reset 语义）；**AC 分栏（GD-3 产品复审新增）**：接入态与授权分栏展示——`_build/sec3.html:249` 原句「**能力声明不等于当前授权**」、`CONTEXT.md:325` 「harness 取数决策（scope 由三元组决定，与 harness 无关）」，就地改挂只改归属空间、不改可读集，接入态列不得暗示权限变化（防新屏制造「改挂实例＝改权限」类新误读）。原第三段「最近注入快照」**降级为「跳到本次运行的 p5」一行链接**——p5 运行详情已显示 `Harness/Agent/责任链/ContextBinding`、p7 已有 Context Manifest 运行投影行，两段式避免「一屏两址」与双处维护同一事实。注入快照的完整面板化留给 D3（P1）承接，不混进 v1。
2. **空态与不确定态（GD-1 高级产品经理条件③）**：一个 harness 都没接时，屏面写「接入向导」而非空表；实例行呈现「**未探测 ≠ 可用**」（`sec3.html` 既有不变量）。GD-3 产品复审补真实实例：`CONTEXT.md:323` 的 M5 契约即现成降级样本——📄 未落地（proxy 侧已消费、harness 侧由用户自研），接入态列须呈现「组件未交付」而非绿。
3. 模型结论（登记）：**MemoryEvent 已有 `harness_id` 字段——确认「记忆携带采集来源 harness」不变量已覆盖差异化点**，外部无一家有此字段；本议题只需把这个既有能力照亮到界面，不新增字段。
4. 接入形态结论（登记）：Connector Contract V1（议题 01）+ MCP 适配已确认；反代零代码形态是**接入通道的一种**而非前提，屏设计必须通道无关（代理/插件/MCP 三种都只体现为「实例+租约+能力声明」行）。
5. 预算化召回的界面语义（自 D3/议题 04）由本屏承接跳转：注入快照详情即「按需融合」的用户可见出口（v1 经 p5 链接承接，不复制数据）。

**建议改动清单（列表级）**
- 原型 `_build/`：新增 `#/harness` 屏（**两段式**如上：实例清单＋会话绑定与就地改挂；注入快照为 p5 深链，不作第三独立段——GD-2 复审修正：初稿此处「三段式」与 ③.1 切段自相矛盾，照旧文实现会复发「一屏两址」；演示数据进统一数据集）；p9 保留为「配置/装配」，新增一列「运行时接入状态」链接到新屏。**新增路由的四处连锁成本（GD-1 前端架构师登记，付费前不得选此路径）**：① `CONTRACT §3` 19→20 路由；② 侧栏 blocks 须逐字一致（`check.sh:81` 硬编码 `len(blocks)==12 and set(anchors)=={20}`，加屏后计数同变，改一条路由＝改全部屏）；③ 新条目须在 §3 图标表登记 sprite id（现 18 id 无 plug/harness；可复用 `i-cpu`/`i-flow`，或 `template-sprite.html` 新增 symbol——后者不破零外部依赖红线）；④ 若把 Harness 登记为**新模块**，§7/§5.0/p12 三处同步 +1 行（16×8→17×8）。**v1 退路（GD-1 给出、我采纳进清单）**：若不愿付 ①③④，两段内容退到 **p9 内新增页签**承载（GD-2 复核实测：`PRODUCT-DESIGN.html:3341` 能力声明表就在 p9「6.9」段内——页签退路是在已有物件旁边加，不是新造），路由决策留给 2a-ui-design——零导航、零矩阵、零 sprite 变更。
...
- `1-requirement`：US 候选「管理员能看到：每个 harness 实例的接入态、每轮注入了什么、当前会话挂在哪个空间并可改挂」；AC 措辞分栏——接入态列不得暗示权限变化，就地改挂只改归属空间不改可读集（GD-3 产品复审条件④）；「未探测≠可用」演示含 M5 样本（GD-3）。
- backlog 映射（GD-1 改锚）：本屏界面需求来源＝**D3 召回解释与调试台（P1）**；A5 配置来源解释（P2）为**次级参照**（GD-3 领域专家：A5 讲配置值来源、贴合度低于 D3，⑤ 题面须如此标注，防被读成并列依据）；**B13（P0）是实现前置**（召回只允许一条被解释的路径），不是本屏的需求来源。依赖登记 ROADMAP §3。
- 不做：真实代理网关实现、`mem:` 指令语法照搬（会话内指令属产品交互大改，超出融合范围，留档）。

## ④ GD 议题门（R16.7 法定减配 3 人）

三票（第 1 轮）：领域专家 ✅（并加强：租约/撤销时间已在字段层，「模型已有、缺屏」比他稿自述更强）／前端架构师 ❌→**GD-2 复审改判 ✅**（两条件核销：①「只追加永不改号」落 `:40` 并升 ROADMAP §1.7 硬规则；②四处成本 `:39` 逐条到位、"加屏后计数同变"写法获其认可；p9 页签退路经其独立实测成立）／高级产品经理 ❌（三条件：改锚 D3/A5＋v1 切两段＋空态/未探测——**已落 ①.2/③.1/③.2/清单**）。GD-2 复审顺带交付两处文本修复（本文件已改）：清单「三段式」→**两段式**（消除与 ③.1 的自相矛盾）；③ 序号唯一化（1–5，③.2 现唯一指向空态条）——产品复审票在 `c3f34e7` 上重列这两条为残留 1/2，实际已在 `38e750c` 闭合。
**GD-3 复审（高级产品经理）**：✅ 转判；其残留 3（`M5` 指源→`CONTEXT.md:323`，已改）、残留 4（AC 分栏防「改挂＝改权限」误读，已落 ③.1/清单）、A5 降为次级参照（GD-3 领域建议，已标）。**本议题 GD 通过（3/3 ✅）**，待 ⑤。
