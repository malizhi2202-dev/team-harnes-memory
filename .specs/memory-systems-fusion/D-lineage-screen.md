...
1. **有入口、无屏（论据重心已按 GD-1 修正；计数口径按 GD-3 产品复审判定）**：`_build/CONTRACT.md` §3 侧栏 19 条唯一真源路由里有 `#/governance/lineage`，而 13 屏**内容层无承载**（`sec5.html:360` 手册还在指示用户去该屏追溯写入结果）。**同类「内容无承载」共 6 条（含血缘自身）**：另 5 条为 `#/library`、`#/scenario/cs-2201`、`#/ingestion/import`、`#/changes`、`#/settings`；**另有 4 条属「同屏/页签承担但地址栏路由串不一一对应」**（`#/project/p-db` 屏上地址栏显 `#/task/t-104`、`#/memory/spaces` 走 p7 页签、`#/permissions`/`#/audit` 走 p12 页签但地址栏只显 `#/team`）——两类性质不同、分开记，防复核按不同口径数出两个答案（产品票实测命令：`grep -rho 'class="url">#[^<]*' _build/proto*.html | sort -u`，本会话独立复跑与两类清单吻合）。本议题不是「兑现唯一承诺」，而是**系统性「导航-屏覆盖缺口」中优先修血缘这一条**。优先理由（价值/依赖论据，非承诺论据）：①§5.0 已把血缘当**带 8 角色可见性的模块**（`sec5.html:69`）；②它是 D-4 消费列与 D-6 撤销传播的**共同数据源**；③p5/p7 内嵌片段承担不了角色可见性的演示。其余 5 条内容无承载＋4 条串不一致的处置在 ⑤ 单列（不得让「补一屏」被读成该类缺陷已结）。
...
1. **补屏方向确认（论据已换轴，GD-1 双票条件）**：不是「兑现导航承诺」，而是价值/依赖——血缘是 D-4 消费列与 D-6 撤销传播的共同数据源，且 §5.0 已把它当带角色可见性的模块；补一屏（或把 §5.14 手册可视化）属兑现该模块的运行时形态。**系统性判定一并登记**：「内容无承载」6 条（含本条）＋「同屏/页签承担但地址栏串不一致」4 条（口径见①.1），本议题只优先修血缘这条，其余 5＋4 条的去留在 ⑤ 单列，防止「补一屏＝该类缺陷已结」的误读。
...
- `1-requirement`：US 候选「任意一条记忆可回答：从哪来、经哪些加工、去了哪里（被谁消费）」+ AC 三态分列（无消费记录/无权查看-隐藏占位/追溯失败，见③.2）；**AC 明确本屏只承接 `#/governance/lineage` 一条路由，其余 5 条内容无承载路由与 4 条地址栏串不一致路由均不在本 change 范围**（GD-1 领域专家条件④＋GD-3 产品 6/4 口径：防「补一屏＝该类缺陷已结」误读）。
...
首轮三票：前端架构师 ✅／高级产品经理 ✅（附带条件：论据换轴+三态分列——已落 ①.1/③.2）／领域专家 ❌（四条件：`captured_from` 投影定性／废 `generation_refs`／权限 AC 三态／悬挂系统性——已落 ①.2/③.3/③.1/清单）。
**GD-2/3 复审（两张回票）**：领域专家 ✅ 转判（四条件核销，并记「get_lineage 增层比初稿两条并列端点更符合 07 §4 单一真源」）；高级产品经理补 6/4 悬挂口径（已落 ①.1/③.1/清单 AC 行）。**本议题 GD 通过（3/3 ✅）**，待 ⑤。
