...
- 会话绑 project P → 取三块；纯聊天未绑 → 只取公共+私有；显式要某 project X → 临时加，先验读权限（白名单兜底）。
## 记忆系统调研融合域语言（2026-09-22 · enterprise-agent-memory-product v0.3）

### 术语表（新增）

- **生成溯源**：一条记忆候选"由谁产生"的四元组——抽取 prompt 版本与生效范围、来源消息区间、抽取模型与参数指纹、生成日志指针；缺任一即"溯源缺失"。
- **机器合并建议**：候选与既有记忆比对后给出的 `ADD / UPDATE / SUPERSEDE / NOOP` 候选加逐条理由；定位是**决策输入**，永不自我裁决（区别于 mem0/Zep 的写入期自动合并）。
- **双时间轴**：现实时间线（内容何时为真 / 何时停止为真）× 系统时间线（我们何时得知 / 何时下架）；本设计仅固化产品表达，Schema 为候选（§10.2）。
- **失效三态分离**：「下架（退出召回）」「删除证明（CopyInventory verified）」「审计保留」是三种独立事实，互不折算；失效一律 `invalidated/superseded` 标记，不物删。
- **superseded_by 关系**：新旧取代是关系表达，不是状态覆盖，更不是删除。
- **证据档位**：引用外部产品事实必标【源码已落地 / 文档声称 / 营销宣传】之一；「未验证」「厂方自称」项不得进入正文结论与任何决策依据。
- **时效快照结论**：「行业零命中」类断言必须带调研日期并服从 §10.5 降级约定；四轴隔离断言已收窄为「harness 一等轴 + 跨 Harness 交接语义无对应物」。

### 已锁决策（新增）

- 竞品基准分数（LoCoMo/LongMemEval/BEAM 等）**不进 PRODUCT-DESIGN 正文**；对比叙事只用可核验的治理语义有无。
- `_research/` 六文件为 v0.3 §1.5 唯一事实源；其结论被文档引用部分已随 2026-09-22 人审「通过」，未验证项例外。
- 裁定更新（2026-09-22 晚，用户逐条裁定）：C1 双时间轴（MemoryItem 现实有效区间 + superseded_by 关系化）与 C3 双速写入（个人直写+团队审核）**已采纳为产品语义**；C2 删除可逆窗口仍留档议题。三者剩余边界：时点查询、存储 Schema、检索内核与时延指标归实现 change。
- Team↔Project 基数 = **1:N owning**（用户裁定原话「一个project仅仅属于一个team」）；redesign-v3 的 N:N 关联口径不再采纳、仅作历史源；裁定不动摇不变量①（归属/关联/引用 ≠ 权限）。
- 接口面 = 通用接口 + 配置化对接（2026-09-23 裁定「提供通用的接口配置对接即可」）；端点命名与协议 Schema 仍归实现 change。
- 数字层默认 = 业内参考值表（§10.1，top_k=10、48h 窗口、TTL 3–365d 等，均带来源档位）；定性为**可配置默认值而非质量承诺**，拓扑/责任人/法务口径继续暂缓。

### 默认行为（新增）

- 再遇"我们和市面产品差异"类问题 → 直接答 §1.5 三卡片（趋同/独有/缺口），并按证据档位补引用，不重新口头对比。
- 竞品事实更新（新增审核门/数据面审计/行动准入出现）→ 先更新 §1.5 对照行与 §1.5.2 卖点降级，再动任何下游文档。
