...
## 9.7 v0.3.2 十项裁决验证（2026-09-23 · 已过审）

十项用户裁决（①–⑤ 09-22 21:45 批次：检索三能力可配 / 双时间轴采纳 / 双速写入 / 模板画像供给 / Team↔Project 1:N；⑥–⑩ 09-23 批次：删除可逆窗口口径 / ChangeSet 统一审核容器 / 接入体验延后成题 / 通用接口配置化对接 / 数字层业内参考默认表）全部落盘。机判证据：

| 检查 | 结果 |
|---|---|
| `grep -c '已裁 2026-09-22'` / `'已裁 2026-09-23\|部分已裁 2026-09-23'` | 13+3 / 7 ✅ 每项裁决至少一处锚点 |
| Team 1:N 原话「一个project仅仅属于一个team」 | ×3（§3.2/§5.3/§10.2）✅ |
| ChangeSet 原话「是的装入进去」 | §10.2 ✅ |
| §10.1 默认值表出处档位 | top_k=10（mem0+AgentCore 双源【源码已落地】）、48h（Google【源码已落地】）、3–365d（AWS【源码已落地】）、≤7k（mem0【文档声称】）、#6627 数字纪律【源码已落地·教训】✅ |
| 时间戳纠错 | 六文件"09-22→09-23"（C2/接入/ChangeSet/接口/数字表批次），FUSION 内不实的"21:58"删除 ✅ |
| 长 chip 溢出回归 | chip 只放短标、正文承载说明；渲染 8 项五档宽度 0 溢出 ✅ |
| 呈审口径修正 | Run/Attempt 行改写「待确认 · 归架构 change」，与"无需产品侧裁决"的口径自洽；用户无遗留待办 ✅ |
| 人工过审 | **通过（2026-09-23 11:42 · 用户原话「通过」）**；顶栏/§0.4/页脚三背书时间并列 |

产物：PRODUCT-DESIGN.html **480,226 B / 4,590 行**，check.sh 全绿。
