...
## 9. v0.3 增量验证（2026-09-22 · AC-9~12 · T-05）

实现已随 P-product 过审交付（提交 `ea269e8`）；本节为验证与登记。重建自 `081387e` 前 `_build/` 当前态，`bash build.sh && bash check.sh` 全绿（471,070 B / 4,561 行 / 外链 0 / 侧栏 12 屏一致 / 47 锚点有序）。

| 检查 | 命令 | 结果 |
|---|---|---|
| AC-9 生成溯源存在 | `grep -c '生成溯源'` | 3（§5.12 步骤 + §6.6 dl 标签 + 附录挂接）✅ |
| AC-10 建议不自我裁决 | `grep -c '不自我裁决'` / 绕过文案 `grep -c '自动应用\|自动合并落位\|无需人工确认'` | 2 / **0** ✅ |
| AC-11 双时间轴与失效表达 | `grep -c '时间线与失效'` / `'invalidated ≠ deleted'` / `'superseded_by'` | 2 / 1 / 4 ✅ |
| AC-11 口径联动 | `grep -c 'approved ≠ published'` | 15（含 §2/§4/§6.7/画廊）✅ |
| AC-12 基准纪律 | `grep -n 'LoCoMo\|LongMemEval\|BEAM\|DMR'` | 仅 1 处、位于 §1.5 引用纪律 callout 的"不进正文"声明句；伴随分数扫描为空 ✅ |
| 真实浏览器回归 | `node _build/render-verify.js` | 8 项全过：JS 错误 0 / 外部请求 0 / 12 壳侧栏未裁切 / 目录跳转 8 项不遮挡 / 页签隔离 5 组 / 主题持久化 / 5 档宽度溢出 0 / 打印隐藏 chrome ✅ |

UAT-09~12（对应 AC-9~12）机器部分已如上验证，仍需人工：

| 编号 | 步骤 | 机器已验证 | 仍需人工 |
|---|---|---|---|
| UAT-09 | 打开 `#/ingestion/triage` 详情 `tri-512` | 溯源四要素 dl 存在（prompt 版本/消息区间/模型指纹/日志指针） | 四要素措辞是否足以支撑裁决判断 |
| UAT-10 | 看合并建议行 | ADD/UPDATE/SUPERSEDE/NOOP 建议 + 逐条理由 + 「仅参考，不自我裁决」标注；无任何自动应用入口 | 建议展示是否会诱导评审人照单全收 |
| UAT-11 | 看 `p7`「时间线与失效」面板 | 双时间轴三行演示 + `invalidated ≠ deleted`、`approved ≠ published` chip | 与 §6.10 清理证据的口径在真实业务下是否读得通 |
| UAT-12 | 通读 §1.5 | 8 行对照表带证据档位；正文零基准分数；快照日期与降级约定在位 | 定位判断是否成立（差异化叙事的主观部分） |

## 9.5 v0.3 未决项补充（隶属 §8 边界声明）

§10.2 新增三条候选（C1 双时间轴 Schema、C2 删除可逆窗口参数、C3 双速写入）维持 `待确认`，不在本 change 冻结；`memory-systems-fusion`（另一 change，⑤ 门待背书）不构成 v0.3 输入。

## 9.6 v0.3.1 增补验证（2026-09-22 · AC-9 对齐 · 已过审）

增补三处：§5.12 溯源缺失规则、§6.6 tri-517 演示行与队列注脚、§8 术语表四行（生成溯源/机器合并建议/superseded_by/溯源缺失）。验证：

| 检查 | 结果 |
|---|---|
| `bash _build/build.sh && bash _build/check.sh` | 全绿（13 屏 / 12 侧栏一致 / 47 锚点有序 / 外链 0 / ≤800KB）✅ |
| `grep -c '溯源缺失' PRODUCT-DESIGN.html` | 5（§5.12×1 · §6.6×2 · §8×2）✅ AC-9 Then③ 自此有原文实据 |
| `grep -c 'tri-517'` | 3（演示行 + 目标空间注 + 队列注脚）✅ |
| `node _build/render-verify.js` | 8 项全绿，增行后 5 档宽度溢出仍 0 ✅ |
| 人工过审 | **通过（2026-09-22 21:01 · 用户原话「通过」）**；状态标已晋级，v0.3（18:56）与 v0.3.1（21:01）背书分列在顶栏/§0.4/页脚 |

至此 AC-9~12 的每个 Then 子句均与已过审文本逐句对应；REVIEW.md 的 🟡 Major 关闭（0 Critical / 0 未决 Major）。
