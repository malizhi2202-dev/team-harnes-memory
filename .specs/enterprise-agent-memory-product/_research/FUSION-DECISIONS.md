...
| F8 | 检索三能力（混合多路 / rerank / 预算装配）全部为用户可配可选，空间默认 + Agent 覆盖，Manifest 记配置溯源 | 用户裁决 2026-09-22（回复 §1.5.2 缺口清单「三种都要 让用户自己配置选择」） | §5.7 配置检索内核步骤；§2.4 ContextManifest 行；§1.5.2 卡 1 已裁；§10.2 |
| F9 | 主题模板 + 角色画像专型开箱供给降冷启动；预装不绕过审核门 | 用户裁决 2026-09-22（「这个需要」）；竞品 topic 模板/profile 先例 | §5.9 开箱供给步骤；§1.5.2 卡 5 已裁；供给形态待实现 change 细化 |

## 二、记候选（进 §10.2，不动概念层）

| # | 候选 | 来源 | §10.2 行 |
|---|---|---|---|
| C1 | MemoryItem/Lineage 双时间轴字段语义（四戳、时点查询） | Zep | 「现实时间线（双时间轴）」——**已裁 2026-09-22 21:45 用户「好的确认」：现实有效区间 + superseded_by 关系化采纳（§2.3/§5.7/§10.2）；时点查询与存储 Schema 归实现 change** |
| C2 | 删除可逆窗口作为保留策略显式参数（对标 Google 48h） | Google/mem0/ChatGPT | 「删除的可逆窗口」 |——**已裁 2026-09-23 用户按建议口径：空间可可选窗；默认不设统一窗口、恢复=受授权动作；legal hold 期窗口无效（§5.15/§10.2）**
| C3 | 双速写入（个人空间直写 + 团队空间审核）缓解在线写摩擦 | Letta/Memobase | 「检索内核与在线写摩擦」——**已裁 2026-09-22 21:45 用户「同意」：采纳进 §2.3/§5.7/§10.2（直写≠对他人可见）** |
