...
> 版本：V3 评审稿 · 2026-09-11 · **本轮交付完整设计与重设计后的 UI/交互原型 HTML，不只是增补文档；不修改生产业务代码，不执行迁移。**  
> 配套原型：[`product-design-v3.html`](./product-design-v3.html)；[完整需求 R01–R14](./REQUIREMENTS.md)；[验收记录](./VALIDATION.md)。旧版 [product-prototype.html](../redesign-v2/product-prototype.html) 仅作历史参考，本轮不改旧目录任何文件。  
> 基线：[旧目录内现有 V3](../redesign-v2/PRODUCT-DESIGN-V3.md) 与 [补充需求](../redesign-v2/06-SUPPLEMENTAL-REQUIREMENTS.md)。本轮只核对这些设计材料，沿用的源码证据未经本轮重新验证；“可复用”不等于端到端已验收。
...
- `现存-可复用` 指基线记录的历史源码/调用证据，本轮未重扫；`恢复` 指历史提出的入口恢复，不保证当前链路可用；`待契约` 需读模型、权限或命令补齐；`目标态` 是设计体验；`建议` 尚未进入承诺范围。
- 本文所有未明确标注为现状的交互、状态机、接口扩展、性能门槛均为**目标规格**；原型数据、图表、表单结果与审批决策均为演示，不证明后台能力已完成。
- V1/V2 是历史设计意图，不继承其完成度；遇到冲突，以本轮 R01–R14 新需求为准；不在本轮修改历史材料。

### 0.1 配套原型的模拟范围与实现边界

- 页面、Tab、文档章节与实际交互数量均**以 [VALIDATION.md](./VALIDATION.md) 实测为准**，不沿用历史固定计数或把规划数量当完成证据。
- **UI 增强范围**：重组侧栏信息架构、重排工作台优先级与区域布局，重设计接入/路由、导入质量、合并审核、快照升级和模板页面，增强四维筛选及专业图谱。文档可以先完成，HTML 必须随后落实同一规格；只加说明、菜单或换标题不算 UI 重设计完成。
- 原型无需真实登录即可探索；`login` 是平台壳之外的独立模拟登录页，顶栏负责进入登录或退出。`settings` 不再包含登录 Tab；旧 `access/*`、`settings/account` 仅作兼容深链。持续显示“演示数据，不连接后端”“不要输入真实凭据”。
- 模拟登录、创建用户、创建个人/服务 Key 只改变浏览器内存；Key 是带演示标记的本地虚构值，不可用于真实鉴权。刷新恢复样本，不存真实密码或密钥。
- 新建任务、导入、纠错草案、审核、索引就绪与发布只模拟低风险可测试结果；未实现动作标“目标规格 / 待接入”，不伪装调用成功。
- 纠错、归档和共享内容变更先进入 ChangeSet；批准不等于发布，演示发布也不修改真实资料。原型不删除文件或数据。
- 策略、模型连接与成员邀请不配置真实服务、不发送邀请；用户与 Key 的模拟创建不应再被写成“仅预览、不创建”。
- Graph 使用有类型和证据标签的示例关系；Analytics 是固定虚构线上指标；incoming memory 质量统计与其独立，不能当实时监控。
- 文档、架构及时序离线可读；角色切换不是生产鉴权，演示审计不是不可篡改账本。仅浏览器实际走查过的行为才进入验收通过记录。
...
本次设计优先连通“选择项目 → 明确任务 → 检查 Agent 装配 → 查看运行 → 评审交付 → 沉淀记忆”，并保留 Wiki、Code、Skill、Chat Memory 的专业操作；新增可信身份、绑定路由、传入记忆、变更审核、快照与轻量模板闭环。
...
|---|---|---|
| Team 团队 | 平台唯一组织实体与公共资产池，`team_id` | Team 就是 Organization，不另设组织对象；不等于 Project，团队成员与项目成员分别授权 |
| Project 项目 | 持续协作现场，名称、owner、visibility、代码锚点 | 与 Team 独立并通过多对多关系关联；关系不授予项目权限；汇集任务、Agent、资产与知识条目 |
| Task 任务 | 可验收的工作目标，目标、负责人、状态、风险、完成条件 | 可归入 Project；声明 Agent 与实际参与者分开；一次任务可有多次 Run |
| Agent | 可独立 CRUD、有 owner 的执行配置，prompt、模型引用、资产装配 | 可关联多个 Team 与 Project；成员关系不替代 Agent 权限，历史 `project_id` 单值仅作兼容 |
| AgentTeam / Automation | Agent 协作组 / 触发配置 | 属于编排，不是组织 Team；现有配置 CRUD 不等于已具备完整调度控制台 |
| Asset 资产（集合概念） | Memory、Wiki、Code、Skill 等可复用知识与能力的统称 | 不设独立一级资产表；发现入口在 Team 资料和各专业工作台，底层类型仍保留兼容 |
...
| Principal / Credential | 用户或服务主体 / 个人或服务凭据 | createdBy 不等于实际操作者；未独立验证用户为 actor_unknown |
| ContextBinding | Task/Run/Session 的工作上下文 | 主项目、参考项目、写目标独立，不读用户 lastProject 决定归属 |
| ChangeSet / Manifest | 统一变更协议 / 原子可见版本引用 | 人与 Agent 共用审核；正文、向量、图谱同版本发布 |
| Snapshot / Template | 规划与实际版本快照 / Agent、Scene、Playbook 模板 | 旧任务固定版本，显式升级；模板不授予权限、不新增运行时 |
...
3. **本轮生效 effective**：特定 Run 从 Task planning snapshot 经当前授权、版本、策略与预算解析后的上下文；显示 `runId / actualSnapshotId / bundleId / resolvedAt`，无 Run 显示“尚无运行快照”，旧 Run 缺证为 unknown；不拿当前装配或 latest 补历史。
4. 生效不能被简化为固定装配的子集：Task 临时选择、Project 默认来源也可能进入执行包；固定装配可能因权限撤销、版本过期或策略而未生效。服务端授权是所有路径的必要条件。
5. “进入执行包”“可召回”“命中”“正文注入”分别标识；Skill 的 mounted、loaded/read、executed、task accepted 分阶段记录。仅注入索引不等于正文已读；缺加载证据为 `loaded=unknown`，不能推断“未使用”或“执行成功”。
...
- **写策略**：`automatic` 在本轮仅允许符合策略、范围不扩大且不改共享事实/行为的低风险去重；其他自动提炼生成候选，共享发布人工审核；`review_required` 先审后写；`explicit_only` 只接受有权用户的明确写入意图，自动提炼不能借审批绕过；`deny` 拒绝。四档策略的历史纯逻辑证据见 E10；本轮低风险限制与发布流程为新增目标，不由该证据证明已实现。
- **纠错语义**：目标采用新版本替代旧版本并保留 `supersededBy`；“忘记”默认生成归档变更草案，审核发布后停止未来召回，不承诺擦除已生成的历史输出。硬删除另走合规方案，本原型不提供真实删除。
...
erDiagram
  ...
  TEAM ||--o{ TEAM_PROJECT : relates
  PROJECT ||--o{ TEAM_PROJECT : relates
...
新版侧栏按“工作 → 执行协作 → 知识与记忆 → 治理与版本 → 组织与平台”重组。登录是壳外独立页面；资产是能力集合而非独立表格；Code 来源与分析合并为一个工作台。旧 route 可做兼容跳转，但不继续制造重复一级入口。分组标题不是权限边界；当前路由高亮、侧栏定位、面包屑和页头保持一致。以下为目标**原型 route key**，不是业务 API。

| 分组 | 原型路由 | 页面职责 / 二级 Tab |
|---|---|---|
| 工作 | `home` | 工作台：继续工作、待处理决策、异常与最近项目 |
...
| 执行协作 | `agents` → `agent` | Agent 列表 → `overview / projects / assembly / memory / runs / policy / versions` |
| 执行协作 | `orchestration` | 手动编排（编辑步骤、人工启动）与自动编排（触发、启停）及多次 Run；不混同组织团队 |
| 执行协作 | `runs` → `run` | 运行列表 → `timeline / context / tools / artifacts` |
| 执行协作 | `templates` | `agents / scenes / playbooks`：内置模板及轻量复用 |
| 独立登录 | `login` | 账号登录、会话失效与登录后进入平台；不放入 Settings |
| 治理与版本 | `ingestion` | `import / quality / experience`：来源记忆输入、自动归纳、归属判断、质量证据与人工待归类 |
| 知识与记忆 | `memory` | 跨来源记忆浏览、四维筛选、来源及变更草案 |
| 知识与记忆 | `spaces` → `space` | 空间列表 → `overview / memories / mounts / policy` |
| 知识与记忆 | `wiki` | 文档、检索、引用与知识图谱 |
| 知识与记忆 | `code` | `repositories / sources / graph / tree / process / query`：仓库、无 Git 上传、代码内容与分析图谱合一 |
| 知识与记忆 | `skills` → `skill` | Skill 目录 → 正文、版本、来源与使用关系 |
| 治理与版本 | `changes` | `queue / review`：统一草案队列、合并差异、冲突与审核 |
| 治理与版本 | `snapshots` | `manifest / upgrade`：构建矩阵、规划/实际快照和升级差异 |
| 治理与版本 | `governance` | `approvals / permissions / audit / evals`；与 changes 同对象互链 |
| 组织与平台 | `team` | 唯一组织入口：Team CRUD、成员、项目关系，以及可点击的团队资料聚合 |
| 组织与平台 | `settings` | `users / keys / models / integrations`：用户 CRUD、API Key、模型与集成；不包含登录 |
| 组织与平台 | `analytics` | 线上调用概况/模型用量，区别于来源记忆质量与 Code 分析 |

“资产”仅作为 Memory、Wiki、Code、Skill 等可复用资源的集合概念，不再提供独立一级资产表。Team 资料聚合 Project、Task、Memory、MemorySpace、Wiki、Code、Skill，并按 Team / Project / Agent / 个人（User）四维组合筛选：同维 OR、维间 AND。
...
### 3.2 V3 UI 重设计：全局壳、工作台与关键页面布局

- 保留现有 Tea Component、腾讯蓝主强调、中性灰底、细边框与高信息密度；生产实现从既有 Tea token 取色，原型蓝色可采用 V2 的 `#2563eb`，不另加营销渐变。[E15]
- **全局壳**：顶部左侧实例与查看范围，右侧模拟身份、明暗主题、文档/原型切换；左侧按 §3.1 五组导航，内容层为面包屑 → 页头/唯一主操作 → Tab → 筛选 → 工作区。模拟状态提示持续可见，查看范围不能冒充执行绑定。
- 参考尺寸：顶部 52–56px、侧栏 224px、内容边距 24px、表格行高 44px、正文 14px；桌面详情默认主内容与关系摘要 2:1，卡片圆角 6px。
- **工作台重排**：页头放“新建任务/导入记忆”清晰入口；下方用紧凑可点击摘要显示待归类、待审核、失败构建/运行；主体左侧约 2/3 为继续工作与待办队列，右侧为当前身份/绑定说明、最近项目及质量缺证提醒。卡片必须带范围并跳真实筛选结果，异常不是装饰统计。
- **团队资料与专业工作区**：Team/Resources 用四个下拉多选组合 Team、Project、Agent、个人（User），并以可点击卡片进入 Project、Task、Memory、MemorySpace、Wiki、Code、Skill；不再维护独立资产表。Code 将来源、内容、代码树、图谱、调用链和查询放在同一页面，使用统一 source revision。
...
- URL 只承载非敏感查看上下文/筛选，不是授权或执行绑定权威；本地最近值不得决定写空间。Team 是可清除筛选，跨团队项目独立验权。Task/Run/Session 绑定以授权服务结果为准，冲突须确认；身份切换隔离缓存，迟到请求不得覆盖新主体结果。
...
Core 是 scope、命令授权、写策略与版本冲突的权威；目标认证区分 credential principal、独立用户证明及委托，服务 Key 不继承创建者私权；当前 `resolveExecutionBundle` 是确定性纯函数，不读库、不掌握授权和终态，目标完整编译需额外的 caller-aware 编排与持久化。[E9]
...
1. 生产登录并确认主体及授权范围；原型可直接探索，或在 access 模拟登录，禁止输入真实凭据；若尚无可访问项目，展示创建权限说明或申请加入，不展示虚构的“0 个任务完成率”。
2. 进入 Project，阅读目标与成员；从已有资产选择 Wiki/Code/Skill，再选择 Agent。跨团队/多项目关系须显式授权，原型可展示关系，后台未接入标目标规格。
3. 新建 Task，填写目标、验收标准、风险与声明 Agent；检查执行包预览并固定 Task planningSnapshotId。无执行契约时按钮标“演示启动 / 待接执行”，不能伪装发起生产 Run。
...
sequenceDiagram
  ...
  T->>B: 解析 Task 规划快照与绑定，重验当前授权
  B-->>T: 固定版本上下文与差异；缺引用暂停，不回退 latest
  T->>L: 创建 Run 并固化 actualSnapshot 与 bundle
...
```mermaid
sequenceDiagram
  participant I as Incoming Memory / 人工编辑
  participant G as 身份 Scope Policy 与绑定校验
  participant C as ChangeSet / 三方合并
  actor H as 有权 Reviewer
  participant P as 派生构建与 Manifest 发布
  I->>G: sourceRefs + context + explicitIntent
  G->>G: 独立用户或服务主体；主项目/参考项目/写目标分别验权
  alt 显式越权 / deny / explicit_only 缺少明确意图
    G-->>I: 拒绝，不 fallback、不写共享空间
  else 歧义
    G-->>I: needs_classification；合法隔离暂存或拒绝
  else 有权且策略允许
    G->>C: baseRevision + proposed + 来源
    C->>C: base/current/proposed 合并；locked 与语义冲突检查
    C->>H: mergedHash + headRevision + 影响与缺证
    H->>C: 人工审核；修改 merged 使旧批准失效
    C->>P: 准备不可变内容与同版本必需索引
    P->>P: 复验权限、审核绑定；CAS 切换 manifest
    P-->>I: published / index_failed / merge_required
  end
```

仅在策略允许、不变更共享事实/操作行为且不扩大范围时，可自动执行低风险去重；其余提炼只生成候选。explicit_only 不能借批准绕过明确意图，Agent 不自批。
审批过期由服务端 expiresAt 裁决，正式超时待评审，不沿用演示倒计时作为真实策略。批准、索引就绪与发布分别显示；旧 manifest 在失败时继续可用。
发布不更新旧 Task 规划快照；新任务选已发布 manifest，旧任务仅经显式升级生成新快照修订与新 attempt。

### 5.5 记忆诊断与质量回路

- 诊断召回：选择 Agent/Project/Task → 输入问题与预算 → 服务端按同一 scope 检索 → 显示命中、过滤、排序与最终注入摘要 → 关联 Memory 来源；不得展示无权候选正文来“解释过滤”。
- 纠错：来源抽屉 → 纠正内容与原因 → 比较版本和影响 → 提交 ChangeSet → 三方合并/人工审核 → 必需索引就绪并 CAS 发布 → 旧版本标替代；并发冲突保留草稿供人工合并。
...
### 6.1 工作台 `home`
- **能做什么 / 布局**：工作台按 §3.2 改为行动优先布局；待归类、待审核、失败构建/运行摘要连接主队列，最近任务/项目可继续，右栏解释身份和绑定；不只复制旧统计卡换标题，所有指标限定当前可见范围。
- **如何操作**：确认查看范围 → 从待归类/待审核/失败摘要带筛选进入 routing/changes/snapshots 或 runs → 打开对象抽屉 → “进入工作区”；新建任务先选 Project，导入转 ingestion；返回工作台恢复范围但不更改执行绑定。
...
- **如何操作**：创建项目填写名称、可见范围、描述和可选仓库锚点，并可关联多个 Team → 查询或进入概览 → 编辑属性、关联/解除 Team、添加项目成员或资产 → 从任务 Tab 新建 Task；删除前展示任务、Run、资产等影响并要求确认或迁移。Team 关联本身不授予项目权限。
...
- **如何操作**：填写标题、描述、验收标准、Agent、风险及绑定 → 保存草稿，创建 Task 时固定 planning snapshot → 启动并跟踪 Run → 接受或退回交付；新版仅通过 snapshots/upgrade 确认新规划修订及新 attempt，变更状态说明理由。
...
- **如何操作**：创建 Agent 时可选内置模板并固定 revision，填写名称、职责、可见性并关联多个 Team/Project → 在 assembly 选择固定装配 → 查询、编辑或解除关系；删除前预览任务、编排、Run 与资产影响并确认或迁移。用“预览下一次运行”检查策略，历史 effective 必须先选择 Run。
- **为什么**：用户既要知道“默认带什么”，也要知道“为什么这次没带”；预览不能覆盖运行事实，修改装配只影响未来解析。
- **边界**：get、固定资产 get/set 存在；详情本身仍为占位；历史默认模板 get/set client 的管理入口仅限 system_admin；新增 templates 是目标，不以模板赋权。当前单项目字段不等于多项目功能已完成。[E4、E5]

### 6.5 Agent 编排 `orchestration`
- **能做什么**：维护协作组、步骤、角色与 Agent；手动模式支持编辑后人工启动，自动模式支持配置触发条件与启停；两种模式均保留多次 Run。Team 组织、AgentTeam 协作组使用不同名称与图标。
- **如何操作**：新建编排 → 选择已有 Agent 并编辑步骤 → 选择手动或自动模式。手动模式检查依赖后人工启动；自动模式设置事件条件并启用/暂停；进入运行记录查看每次 Run。“预览执行”只是启动前辅助，不代表执行能力。
- **为什么**：将执行协作从组织成员管理剥离，避免管理员误以为加团队成员就是装配多 Agent 流程。
- **边界**：AgentTeam/Automation 组件已有 list/create/member 操作；基础手动启动、自动触发启停和运行记录是目标需求。可视 DAG、复杂重试策略与冲突校验仍为建议扩展，不声称当前已存在。[E7]

### 6.6 记忆中心 `memory`
- **能做什么**：统一浏览、搜索、纠错与来源追溯；保留 Chat Memory L0–L3 工作视角；支持 team/project/user/agent 四维任意组合，并保留 domain、层级、来源和状态，支持检索试调入口。
- **如何操作**：先选范围再搜索 → 打开记忆抽屉查看来源/版本 → 选择“创建纠正/归档草案” → 填原因 → 到 changes 审核发布；导入转 ingestion，选来源与目标上下文，不默认公开。
...
### 6.8 资产集合（无独立一级页面）
- **能做什么**：资产是 Memory、Wiki、Code、Skill 等可复用资源的集合概念。发现入口放在团队资料及各专业工作台，不再单设资产表；团队资料还同时聚合 Project 与 Task。
- **如何操作**：在 Team/Resources 用 Team、Project、Agent、个人四维组合缩小已授权集合 → 点击 Project/Task 或 Memory/Wiki/Code/Skill 卡片进入专业页 → 在专业页查看来源、版本与装配关系。
...
- **如何操作**：选择来源/导入 → 等待解析完成 → 查内容或提问 → 从引用回到原文 → 需要复用时回资产关系区装配；graph 切显式链接、向量相似、知识关系三类，分别查看原文锚点、模型/阈值、断言证据。
- **为什么**：知识可信度取决于出处和处理状态，不是仅显示“导入成功”；同步失败不能丢掉最后可用版本。
- **边界**：沿用 WikiSourcesPanel/useWikiSources 的来源和内容能力；三类 graph 使用独立图例与证据，向量近邻不是事实支持；统一版本锁定、Run 级引用仍待契约。[E17]

### 6.10 Code 与代码图谱统一工作台 `code`
- **能做什么**：在同一 Code 工作台管理 Git 仓库或无 Git 上传来源，并完成代码内容、搜索、代码树、关系图谱、调用链和问答；所有 Tab 共享同一来源与 revision。
- **如何操作**：在 Code 登记仓库并同步 → 检查索引状态 → “进入代码分析” → 在分析/图谱间切换 → 打开文件/符号引用；失败重试保留已可用索引。
- **为什么**：来源管理和分析虽然职责不同，但用户围绕同一代码来源工作；合并一级入口可以避免 Code 仓库与代码分析重复选源，同时仍用 Tab 保留专业边界。
...
- **为什么**：异常定位需要因果链，不是整段 JSON；“回放”是读事件，不会再次执行工具；context 显示 planning/actual snapshot 差异，旧 trace 缺证为 unknown。
...
- **如何操作**：筛待审/风险 → 打开影响详情 → 进入同一 ChangeSet 填理由批准或拒绝 → 检查构建与发布结果；权限修改先看目标和作用域，审计只读；评测需选固定数据集与候选版本。
...
- **为什么**：线上用量分析服务运营与排障，`code` 的分析 Tab 则研究代码；名称、导航、数据源与空态必须分开。
...
- **能做什么**：Team 是唯一组织管理入口，提供 Team CRUD、团队资料、Project 关联以及把已存在 User/Agent 加为成员；Settings 只管理用户、API Key、模型和集成。登录是独立页面。
- **如何操作**：从独立登录页进入平台；在 Settings/Users 创建用户（不自动加入 Team/Project），再到 Team/Members 添加成员。Team/Resources 可点击进入项目、任务和各知识工作台；Settings 继续管理 Key、模型与集成。
- **为什么**：Team 是组织事实，Settings 是配置动作；“模型连接成功”不等于已授权全部 Agent 使用，也不等于 Analytics 数据源可用。
- **边界**：成员、API Key、模型及工具源已有页面/client；统一平台设置壳与细粒度权限体验为目标。原型支持用户/Key 的内存 CRUD；Team 添加成员不创建用户，不采集真实密钥。[E1、E7、E19]

### 6.16 独立登录与平台账号 `login`、`settings/users,keys`
- **能做什么 / 布局**：`login` 是无侧栏、无业务范围筛选的独立登录页；登录后顶栏显示身份与退出。Settings/Users 提供用户 CRUD，Settings/Keys 提供 API Key 生命周期；API Key 不替代用户会话。
- **如何操作**：从顶栏进入登录页 → 使用演示账号登录 → 进入平台；顶栏可退出。用户页独立创建/编辑/删除虚构用户，Team 成员页只选择已有用户；Key 页管理演示 Key。
- **为什么**：持有凭据不等于知道真人身份。服务 Key 未验证用户时 verifiedUserId=null、actor_unknown，不继承创建者 private。
- **边界**：只修改内存，刷新重置；显著提示不要输入真实凭据。真实密码哈希、会话、限流、轮换及 Harness 鉴权兼容均未实现。

### 6.17 上下文路由 `routing/resolve,bindings`
- **能做什么 / 布局**：resolve 左栏输入凭据和 Task/Run/Session，右栏展示授权结果与理由；主项目、参考项目、写目标分成三个明确区块，底部确认栏只提交当前方案；bindings 以 boundTo/revision 为核心列表，不混成用户最近项目。
- **如何操作**：选择样例上下文 → 校验 Run/Task/Session 与显式选择 → 分别确认 primaryProjectId、referenceProjectIds、writeTargetSpaceId → 保存模拟绑定。
- **为什么**：主项目不是唯一读取源，参考权限不是写权限；两个 Session 不受用户最近项目干扰。LLM/仓库路径只是 hint，不授予身份或权限。
- **边界**：显式无权目标直接拒绝，无 fallback；冲突返回 context_conflict，多候选须确认，无合法隔离区则拒绝暂存。旧 Run 不重绑。

### 6.18 传入记忆 `ingestion/import,quality,experience`
- **能做什么 / 布局**：import 用“选样例 → 覆盖/去重 → 归属确认 → 候选”步骤区；quality 上方分列 known/unknown/denied 与覆盖，下方问题表配证据抽屉；experience 按已验证/未验证分区。只分析 incoming memory，不加探针或执行工具。
- **如何操作**：选样例 → 查看来源/覆盖/截断 → 看归属和去重 → 查看质量证据 → 生成经验候选并进入 changes。
- **为什么**：摘要与衍生记忆不是独立运行事实。以 sourceSystem/session/message/call 去重；重复导入与 derived/injected 回灌不增加独立证据。
- **口径**：Tool/MCP 失败率仅 knownFailure/(knownSuccess+knownFailure)；unknownOutcome、未知分母摘要、policyDenied 分列，零分母“—”。Skill mounted、loaded/read、executed、task accepted 分开，缺证均 unknown。
- **边界**：循环判断排除合法分页、进度轮询、有界重试；根因标 confirmed/hypothesis/missing_evidence。validated_example 必须有输入、环境、结果及验证/验收证据，否则 unverified_proposal；共享发布人工审核。

### 6.19 变更治理 `changes/queue,review`
- **能做什么 / 布局**：queue 是带作者/风险/状态/版本的工作列表；review 顶部固定 target、mergedHash/headRevision，主体为 base/current/proposed 三方差异与独立 merged 预览，侧栏列 locked/语义冲突/来源；审核理由及动作栏与发布状态分开。
- **如何操作**：保存各自草稿 → 三方比较 → 人工裁决 → 批准 mergedHash+headRevision → 构建 → CAS 发布；冲突保留草稿后重合并、重审核。
- **为什么**：非重叠文本也可能事实矛盾；locked 不自动覆盖；修改合并结果或 head 前移后旧批准失效。不能 last-write-wins。
- **边界**：模拟并发场景不是实时多人协作或真实事务；不承诺 CRDT。approved、indexing、published 独立，Agent 不自批。

### 6.20 快照与升级 `snapshots/manifest,upgrade`
- **能做什么 / 布局**：manifest 页并列“当前可见/候选待发布”，下方为文本/向量/图谱必需项构建矩阵；upgrade 页并列旧/新规划与 Run actual 差异，底部列权限/行为影响及新 attempt 确认，不只显示一个版本数字。
- **如何操作**：检查全部必需索引 ready → 复验授权与审核绑定 → CAS 原子切可见 manifest；升级对比差异并确认，生成新规划修订与新 attempt。
- **为什么**：读者经已发布 manifest 读取，不分别取 latest；索引失败保留旧版本，旧任务不自动漂移。原子可见不等于跨库事务已实现。
- **边界**：历史引用不得 GC；缺快照/版本标 unknown 或暂停。撤权仍实时生效，已注入内容不可撤回；暂停受影响 Run，重建授权上下文后新 attempt 继续。

### 6.21 内置 Agent 与轻量模板 `templates/agents,scenes,playbooks`
- **能做什么 / 布局**：agents/scenes/playbooks 保持同一目录壳；左侧筛用途与适用范围，中间模板列表，右侧显示 revision、输入输出、依赖权限和步骤预览；Agent 新建抽屉可选内置模板，Scene 定义情境，Playbook 定义步骤与分支。
- **如何操作**：检查 revision、来源、输入输出、权限依赖 → 装配到现有 Agent/Task plan；修改或从记忆提炼均进入 ChangeSet。
- **为什么**：复用配置与经验，不新增重型 Scene/Flow 引擎；模板名称不带隐含权限，相同名称不自动合并。
- **边界**：R09 暂按 Coding Agent 工作流（定位、方案、patch candidate、测试证据、人工评审），原意待确认；无真实测试与 Git 证据不标已验证或已提交。

## 7. 通用交互规格

- **筛选**：列表默认当前 Team、全部可见 Project；teamIds/projectIds/userIds/agentIds 可独立为空、单选、多选，同维 OR、维间 AND，空维不限制但绝不扩大授权；关键词与类型/状态按 AND 组合，切范围清页码；提交后在 URL 保存条件，显示命中数或“总量未知”；“清除”只清当前列表筛选。
...
角色名称是展示语言，不替代服务端权限；历史证据中 `system_admin` 是全局身份，TeamRole 为 admin/member/reviewer，ProjectMember 为 manager/member；目标 viewer 是只读体验映射，不能直接写入旧枚举。[E3、E19]
...
此表是目标最小权限设计；实现时逐动作对照现有 Core，差异建立契约任务，不靠隐藏按钮“修复”授权。未知 role 不派生任何权限；只读体验仍须逐对象验权，未知 capability/operation 拒绝。
计数、搜索建议、关系数量和错误文案同样要授权；不可见对象与不存在对象使用一致外显信息。已知且公开的功能可显示“无权限”，私有对象不暴露 owner/名称以解释拒绝。
凭据、权限、工作上下文三层分离：个人 Key 共享不能证明实际用户；服务凭据未独立验证用户时 actor_unknown，只用显式服务共享权限；创建者权限不继承。代理用户必须验证身份及委托，以凭据 scope、用户权限、ACL 与委托范围交集执行；不信 body.userId。
跨团队项目成员无需先加入 关联 Team；筛选先授权后匹配。Harness 旧客户端缺用户证明不能自动获得 private，需返回 identity_required；正式兼容需测试现有认证头与错误格式。
...
目标 ChangeSet：draft → proposed → merging → needs_review → approved → indexing → ready → published；分支 conflicted/rejected/merge_required/index_failed/revoked。审批绑定 mergedHash+headRevision，发布前 CAS 失败必须重合并审核。
目标 Memory：经对应变更发布后 active 可转 superseded、archived、quarantined；恢复须检查来源和权限，历史版本不可通过恢复静默覆盖新版本。Approval 分 pending/approved/rejected/expired/cancelled，并独立显示 executionStatus。
...
| Memory `remember/correct/forget/restore` | spaceId/sourceRef/content 或 memoryId/version/reason | 共享变更生成 ChangeSet，批准不代表发布；低风险自动去重受策略限制 |
...
目标 ExecutionBundle 至少固化 context、Agent revision、资产版本与 injectionMode、空间来源/读写策略、工具描述版本、policyDecisions、sourceChain；当前 `direct/summary/tool/reference` 与目标 `index/full/tool` 不能按字符串盲映射。[E5、E9]
bundleId 应由规范化配置内容稳定 hash 得到，不把 resolvedAt 这种观测时间纳入配置 hash；Run 另存时间、快照 revision。没有真实命中事件时仅证明配置，不证明某条记忆实际被模型使用。
新增 Principal/Binding/IncomingMemory/ChangeSet/Manifest/Snapshot/QualityReport/Experience 字段详见 [REQUIREMENTS §12](./REQUIREMENTS.md#12-字段与操作契约草案)。Task 固定 planningSnapshot，Run 固定 actualSnapshot；发布索引来自同一 mergedHash，升级以 expectedTaskRevision 校验，绝不回填旧 snapshot。
...
| 阶段 | 可独立交付的范围 | 依赖与放行条件 |
|---|---|---|
| D0 本轮设计与 UI 原型 | 文档可先落地；同轮重设计 HTML 导航、工作台、关键页及交互状态；保留旧原型 | 文档完成不代替 UI 完成，逐页截图/操作证据见 VALIDATION.md；不宣称业务上线 |
...
| U1 正式工作区接入 | 将 D0 新导航与 Project/Agent/Task/Memory/Asset UI 接真实服务 | 后端接入后逐页放行；不是把本轮原型 UI 重设计推迟到后期 |
...
实际验证记录见 [`VALIDATION.md`](./VALIDATION.md)；以下复选框保留为待验收模板，不作为本 Markdown 编辑任务的测试通过声明。

- [ ] **UI 实际落地**：五组导航按 §3.1 重组；登录独立、Team/Settings 各一个入口、资产与 Analysis 不再占一级菜单；团队资料四维筛选、Code 合并工作台、自动归属/人工队列、三方合并和构建矩阵均具专属布局。
- [ ] **布局证据**：为工作台、team/resources、memory、code/graph、review、manifest/upgrade 记录实际 route/Tab、视口、明暗主题、截图与操作结果；桌面区块层次清晰，窄屏变单列/Tab 切差异，审核动作不遮挡正文。
- [ ] **状态证据**：对归属歧义/拒绝、筛选零命中、图谱缺索引、加载缺证、locked/语义冲突、审核过期/CAS 冲突、构建失败重试逐项观察；按钮禁用有原因，错误保留草稿、unknown 不写 0，恢复后对象与队列同步。
...
- [ ] 模拟登录、用户/Key 创建与编辑有校验、取消、内存反馈和刷新重置；无真实登录仍可探索。纠错生成草案，批准不直接发布，缺索引/CAS 冲突阻止发布，未调用真实写接口。
- [ ] accessible、mounted、effective 三层文字与字段同时成立；未运行 Agent 不显示伪造生效证据，Analytics 不展示代码分析替身。
- [ ] 320/768/1024/1440px、键盘、焦点、错误/空态逐项走查；页面无网络依赖的核心导航可离线使用；旧 [product-prototype.html](../redesign-v2/product-prototype.html) 独立保留，本轮不修改其内容、布局或交互。
- [ ] 文档区分沿用的历史静态证据与本轮目标，不伪称重新扫描源码；R01–R14 按 REQUIREMENTS §13 验收，包括 actor_unknown、拒绝无 fallback、Session 隔离、快照固定、未知分母与经验验证；原型测试结果另附证据，本文复选框本身不代表已经执行测试。
...
业务切片需实际运行项目已有 typecheck/build、契约/权限/单元测试与浏览器验收；本轮仅核对历史设计与需求文件，不将这些未来命令描述为已通过。
性能门槛候选：overview 首屏不超过 3 个业务请求、后台聚合不按列表项 N+1、交互反馈不等待完整长任务；精确延迟 SLO 在固定数据量与测试环境评审后确认。

## 13. 参考证据、差异更正与待决策

### 13.1 沿用基线的源码证据（本轮未重新扫描）
...
- [历史 V3](../redesign-v2/PRODUCT-DESIGN-V3.md)、[R01–R14 原需求](../redesign-v2/06-SUPPLEMENTAL-REQUIREMENTS.md)：本轮直接读取基线，新需求冲突优先；旧目录不修改。
- [早期提案](../redesign/00-PROPOSAL.md)：保留先契约后界面、恢复入口、防假完成原则，不继承历史计数。
- [对象与契约](../redesign-v2/02-TARGET-DOMAIN-AND-CONTRACTS.md)、[调用时序](../redesign-v2/03-CALL-SEQUENCES.md)：继承三层装配与服务边界，以当前身份、ChangeSet、manifest 与 snapshot 规格修正冲突。
- [界面恢复](../redesign-v2/04-UI-RESTORATION-AND-REDESIGN.md)、[历史交付计划](../redesign-v2/05-DELIVERY-BACKLOG-AND-ACCEPTANCE.md)：保留专业能力，不将历史验收当本轮证据。
- GitNexus、Obsidian Graph、GraphRAG、Git merge 参考及采纳限制见 [REQUIREMENTS §15](./REQUIREMENTS.md#15-官方参考与采纳限制)；未新增网络核验，不声称集成对应引擎。
...
3. **目标**增加 Agent 多项目差异装配与版本比较，按 R10/R11 明确建模；可视编排仍为建议，不新增重型运行时。
...
原型创建归档变更草案，经模拟审核发布后停止未来召回，不重写历史 Run、Task 规划快照或报告。正式产品的合规删除必须另行定义源文档、索引、缓存、派生产物和审计保留策略，不能用归档冒充彻底删除。
...
- **证据优先的详情抽屉**：把“结论 → 记忆 → 来源 → Run”放在同一追溯路径；导航是否已演示以 VALIDATION.md 为准，正式接入仍待 provenance 契约。
- **下一次运行的影响预览**：装配或策略变更前展示受影响 Agent、空间和来源；不要修改历史 Run。本轮设计包含 manifest 与 planning/actual snapshot 差异，原型实测范围以 VALIDATION.md 为准。
...
Team、Project、User（owner）、Agent（used_by）四个维度改用可展开多选菜单，不再把所有选项同时铺开。菜单中显示已选数量并支持滚动；提交后以非敏感参数持久化 URL。关系语义保持：同维 OR、维间 AND，先做授权裁剪再筛选，User 不等于当前登录者，Agent 不等于资源 owner。

### CodeGraph 按来源分图

代码图谱按一个 Git 仓库或一个无 Git 上传来源划分分析单元。Git 图谱必须展示 `repoId + commit`；上传目录包使用上传批次和内容摘要；上传单文件使用文件内容摘要。来源选择是图谱工具条的一部分，节点、边、版本和索引状态均只来自当前来源；不跨来源拼图，不为上传件编造仓库、分支或 commit。新的上传或 commit 生成新 revision，旧 revision 保留引用。

### Team 管理与共享边界

Team 管理流程为“创建 Team → 设置默认可见性 → 添加 User 或 Agent 成员 → 关联 Project → 分配项目权限”。界面始终标明成员类型：User 使用 Team 角色，Agent 是服务成员；Agent 不能因加入 Team 自动获得写入、审批或其他工具权限。Team 级资料在 ACL 允许的前提下对全体 Team 成员可查看；Project 资料、User 私域和敏感资料仍按各自 ACL 与写策略隔离。切换 Team 或 Project 会同步更新 URL、面包屑、内容范围、对象/Tab 和左侧导航高亮，旧对象上下文不继续沿用。

## 本轮纠偏：平台设置与核心对象管理

### 统一组织实体

Team 即组织，不能在侧栏再并列一个 Organization 管理入口。`team` 页面是唯一组织管理页，提供团队 CRUD、用户/Agent 成员、团队资料和关联项目。Team 级资料在可见性设为 Team 时对团队全体成员可查看；Project、User 私域和敏感资料仍按 ACL 隔离。

### 平台设置

`settings` 是全局平台设置，不承载登录，包含以下管理：

1. **用户管理**：独立创建、查询、编辑、删除用户。创建用户不自动加入任何 Team；加入 Team 是 Team 管理员的单独动作。
2. **API Key**：创建、查询、作用域、轮换/撤销和安全说明；真实秘密不进入原型数据。
3. **模型配置**：Provider、模型、用途、连接测试与状态；配置模型不自动授权 Agent 使用。
4. **集成设置**：Harness、知识来源、工具与遥测连接；集成配置不承担登录或资源归属裁决。

### Team、Project 与 Agent

Team 与 Project 是独立维度，可多对多关联；关联只表示协作关系，不直接等价于项目读写权限。Project 列表/详情必须提供创建、查询、编辑、删除以及团队关联。Agent 列表/详情必须提供创建、查询、编辑、删除以及团队/项目关联。删除必须显示影响范围和确认；生产实现应阻止危险级联或要求迁移，原型只模拟安全确认。

### 编排与接入归属

Agent 编排不是单一模拟运行按钮：工作流需支持手动启动、自动触发、启停、步骤/Agent 编辑和多次 Run。治理与版本中的记忆归属流程负责导入后自动归纳，并在 Team、Project、Agent、个人四类目标中判断归属；不能可靠判断的进入人工待归类队列。平台设置只负责用户、Key、模型和集成配置。LLM、仓库路径或最近访问记录只能作为提示，不能授予身份、权限或最终归属。

## 15. 最终信息架构收敛

1. **登录独立于平台设置**：未登录时顶栏进入 `login` 独立页面；该页不显示业务侧栏或 Team/Project 范围。登录后进入工作台，顶栏显示当前用户与退出。Settings 仅保留用户、API Key、模型、集成。
2. **消除重复入口**：侧栏中 Team 和 Settings 各一个；Team 是唯一组织实体。旧 `access`、`assets`、`analysis`、`routing` 仅可作为兼容深链，不再占一级导航。
3. **团队资料是可操作聚合页**：Team/Resources 汇集 Project、Task、Memory、MemorySpace、Wiki、Code、Skill，所有结果可进入对应专业工作区，不再展示不可点击摘要。
4. **四维组合筛选**：四维固定为 Team、Project、Agent、个人（User）。每个维度使用折叠多选，同维 OR、维间 AND；空维度不限制，但服务端授权永远先于筛选。
5. **Code 合并**：Code 的仓库与上传来源、代码内容、关系图谱、代码树、调用链、查询问答位于同一工作台，共享 sourceId 与 revision；Git 使用 repo+commit，上传使用内容摘要。
6. **记忆归属进入治理**：传入记忆先自动归纳，再在 Team、Project、Agent、个人四类合法目标中判断归属。高置信且策略允许的结果进入后续变更流程；多候选、混合范围、缺少上下文或权限不确定时生成“待人工处理”，不得静默 fallback。
7. **资产是集合，不是页面**：资产表示 Memory、Wiki、Code、Skill 等可复用资源，不再提供独立资产表；发现与管理发生在 Team/Resources 和各专业工作台。
